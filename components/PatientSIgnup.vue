    ...
    <div v-show="activeStep === 'base'" class="xl:grid grid-cols-2 gap-x-20 w-10/12 mx-auto pt-20">
      ...
      <div class="pt-2 space-y-10 w-full">
        ...
        <div v-show="activeStep === 'base'" class="space-y-8 xl:space-y-6">
          ...
          <div class="w-full">
            <button
              :class="[!isStepOneFormValid || processing ? 'cursor-not-allowed opacity-25' : '']"
              type="button"
            ...
            <p class="font-light text-sm pt-3">
              Not a professional? <nuxt-link
                to="/signup-options"
                class="text-[#0C9609] font-semibold"
    ...
    <div v-show="activeStep === 'upload'" class="max-w-2xl mx-auto space-y-8 rounded-lg bg-white p-4 border mt-16">
      ...
      <div v-if="selectedProfile" class="w-full">
        ...
        >
          ...
          <input id="utilityBill" accept="image/*,.pdf" type="file" class="hidden" @change="handleProfileUpload">
        </label>
      </div>
      ...
      <div class="text-gray-700 sm:col-span-2">
        ...
        <div>
          <ul
            v-if="showDropdown && !loadingLocations && searchResults?.length"
      ...
      <div class="w-11/12">
        <button
          :class="[!isStepTwoFormValid || processing ? 'cursor-not-allowed opacity-25' : '']"
          type="submit"
...
export default {
  ...
  methods: {
    ...
    handleSignup () {
      ...
      this.$axios
        .post('https://medinizebackend.onrender.com/patient/signup/', this.form)
        .then((res) => {
