  ...
  <div class="md:flex md:flex-row md:justify-between md:space-x-6 py-3">
    <main class="md:w-5/12">
      ...
      <div class="bg-white rounded-md p-6 lg:p-10 space-y-6">
        ...
        <button class="bg-[#086406] text-white py-[.8em] px-3 mt-4 rounded w-full">
          Book ambulance
        </button>
      </div>
    </main>
    <div class="md:w-7/12 md:mt-14 mt-6 space-y-6">
      ...
      <p class="text-gray-500">
        The general rule to remember in first aid situations is to prioritize safety. Call for help, Keep calm and provide basic care.
      </p>
      <div class="space-y-6">
        <div>
          <div v-for="(summary, index) in treatments" :key="index" class="mb-4 p-4 border rounded-md shadow-sm">
            <div class="cursor-pointer flex justify-between items-center" @click="toggleSummary(index)">
              <p class="font-bold text-gray-950">
                {{ summary.title }}
              </p>
              <p>
                <img v-if="summary.open === false" src="@/assets/icons/thick-plus.svg" alt="plus">
                <img v-if="summary.open === true" src="@/assets/icons/thick-minus.svg" alt="minus">
              </p>
            </div>
            <div v-if="summary.open" class="mt-2">
              <p> {{ summary.urgent }}</p>
              <p class="text-gray-600 leading-loose">{{ summary.description }}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    toggleSummary (index) {
      this.$set(this.treatments, index, {
        ...
        open: !this.treatments[index].open
      })
    }
  }
}
